      ...
      <div className="max-w-296 mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {billingPlans.map((plan) => (
            ...
            >
              <h3 className={`text-[22px] md:text-[18px] lg:text-[22px] font-bold font-manrope mb-2.75 ${
                plan.featured ? "text-white" : "text-gray-900"
              }`}>
                {plan.planName}
              </h3>

              <p className={`text-sm md:text-[12px] lg:text-sm mb-6 font-manrope min-h-11 ${
                plan.featured ? "text-pink-100" : "text-gray-500"
              }`}>
                {plan.tagline}
              </p>

              {/* Displayed price based on billing cycle */}
              <div className="mb-6">
                <span className={`text-[56px] md:text-[40px] lg:text-[56px] font-semibold ${
                  plan.featured ? "text-white" : "text-gray-900"
                }`}>
              ...
              {/* Action button */}
              <button className={`w-full py-3 rounded-lg font-semibold text-[16px] mb-8 transition-colors duration-200 font-manrope ${
                plan.featured
              ...
              <ul className="space-y-3" role="list">
                {plan.capabilities.map((item) => (
                  <li key={item.feature} className="flex items-center gap-3">
                    ...
                    <span className={`text-[16px] font-medium font-manrope ${
                      item.available
